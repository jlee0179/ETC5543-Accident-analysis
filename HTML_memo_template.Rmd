...
Out of 23 depots, only 4 don't have an incident. Drivers of depots located in NSW and Melbourne have recorded more than half the MVA accidents. For NSW, Revesby and Taren Point are the depots with most drivers exposed to risks. Despite that NSW has the most depots with incidents, drivers with depots in Melbourne are involved with the most incidents. Doncaster, North-Fitzroy, South Granville and Thomastown are noted with the most danger. 

The discrepancy exists between states, the drivers with depots in WA have much lower incidents, compared with NSW and Melbourne. This may lie in the differences in population. Even though a very small portion of drivers in WA involved had incidents, still, 75% of drivers in Joondalup depot have encountered MVA incidents.
...
The results for serious incidents are pretty close to that for the MVAs with a smaller scale. When narrowing down to slight incidents, only a few depots in Melbourne and one in WA appear. Again, over half of the drivers of Revesby and Taren Point depots had slight incidents. The comparison with the previous graph implies that the majority of MVA incidents drivers encountered in depots in NSW and WA are serious.
...
The absence number for Queensland is due to the lack of data in the data set. Gender differences exist in the data. In NSW and Melbourne, drivers' gender ratio is around 10 to 15 males per female, while it is approximately 6 in WA. Although more male drivers had MVA incidents than females in the three states, the numbers are only of slight differences. The ratio of MVA incidents between genders is nearly 1:1. In short, the proportion of female drivers having incidents is unexpectedly high.
...
All drivers are grouped by their corresponding age group. The plot presents that of the total drivers within each age group, how many of them had been recorded with MVA incidents. Neither the youngest nor the eldest drivers are the most dangerous. The 31-40 age group has the most drivers with incident records. As the age groups go up until 51-60, the number starts reducing.
...
The outcome for serious incidents is roughly the same as that for the MVAs, except on a lower scale. The proportion of drivers for slight incidents exhibits a left-skewed distribution, referring to the tendency towards the older age groups. The proportion grows until 61-70 as the peak, following a decline afterward. 
...
From the graphs above, it is hard to determine the association between age and incidents as neither positive or negative trend presents. ANOVA test takes the age group of 20-30 as the baseline to allow comparisons with the means of other age groups. The p-values are small, confirming the significance of the age groups and the association with incidents.
...
Tenure is derived from the interval between an employee's starting day and today's date in years, if the driver is currently in Transdev. For those who have quit, their tenure will be the interval between the starting and the employment termination dates.

Based on the graph below, a positive trend exists that the proportion of drivers having MVA incidents grows with the increase in driving experiences. This might be misleading, when a driver takes more shifts, his or her exposure to the risks would increase. The result is not surprising that those with 41-60 years of experience had all been involved in incidents. 
...
The outcome overturns when breaking down the total employed years into yearly form, it records the specific year where a driver encounters incidents and aggregates them by years. The previous positive trend now becomes a positive-skewed distribution where the tenure and incidents appear to be negatively correlated. Most incidents are recorded within drivers' first three years of experience. Strictly speaking, the first ten years of drivers' employment holds a great portion of incidents among the entire count which is worth to be discussed.
...
tidy(ten_fit) %>% kable() %>% kable_styling(full_width = FALSE)
```
...
Monday is the peak with the most incidents, whereas Sunday is the trough. The number of incidents that occurred on weekdays is more even and exceeds those on weekends. This can be linked to the heavy usage of private vehicles and public transport for commuting, together resulting in the busy traffic.
...
The hourly graphs illustrate the earlier statement regarding peak hours for traffic. Peak hours, e.g. 7-8 am and 3-5 pm, are found with the most incidents on weekdays. On weekends, the peak with the most incidents is around 3 pm. The variation in incident number has been much narrower on weekends. Nevertheless, a common point between weekdays and weekends is that incidents tend to reduce in the evening. 
...
daynight_incidents_mva %>%
  ...
  labs(x = "Daytime/night", y = "Total claim number", title = "Total MVA claims made during daytime and night")
```
...
Light rain is defined with precipitation less than 2.5 mm ([Wikipedia, n.d.](https://en.wikipedia.org/wiki/Rain)), so is set as the baseline here to distinguish between little/no rain and rain. The hourly rainfall distributions for Melbourne and NSW are plotted with and without MVA incidents. They share a similar shape with the incident and non-incident peaks around 3 mm, which is not considered heavy rain. The downward-sloping curve for Melbourne isn't as smooth as that for NSW. For Melbourne, incidents aggregate closely with 4.5 mm of precipitation. Additionally, its tail towards the right end is slightly higher than that for NSW. This reveals that rain has more effects on incidents in Melbourne.
...
The distributions for Melbourne and NSW are approximately the same when there is little/no rain. The only difference observed is the taller peak for incidents in NSW, more incidents occurred in NSW under the condition with little/no rain.
...
The following outputs are the summary of precipitation with and without incidents, respectively. The overall distributions lean towards little/no rain for both conditions, having 0s dominate the 6 numbers. The only two differences lie in the mean and maximum. The mean rainfall is slightly higher when incidents occur, however, the maximum is lower. Since it is not reasonable to say that incidents have an impact on precipitation, rather, we discover whether an association exists between them. 
...
The nonparametric test resolves the non-normality issues and confirms the effect of precipitation on incidents with a small p-value. 
...
Similarly, the hourly measured temperature is plotted in distributions with and without the occurrence of MVA incidents. The shapes of incidents follow closely after the non-incidents for both Melbourne and NSW where incidents are positioned towards the higher temperature. The peak of incidents is 15 degrees for Melbourne, whereas it is around 20 degrees for NSW. A large portion concentrates between 10 and 25 degrees in Melbourne and 15 to 25 degrees in NSW. Although on a smaller scale, the distribution for Melbourne is relatively right-skewed, compared with NSW. This indicates the common occurrence of incidents on hotter days in Melbourne.
...
Both temperature distributions for incidents and non-incidents seem approximately normally distributed, thus can be proceeded to t-test for significance testing.
...
The p-value obtained from this two-sample t-test is again less than the threshold. Therefore, the null hypothesis is rejected, an association indeed exists between temperature and incidents. Specifically, the "mean in group NO" and "mean in group Yes" prove the statistical difference with a higher mean value for incidents.
...
The graph above plots the distribution of drivers' driving hours against the hour they ran into MVA incidents. The trends and patterns vary greatly from day to night time. During the daytime, a large portion of incidents occurred within the 3rd hour of driving which is the peak. Driving hours between 7 to 10 hours are also found to have accumulated many incidents. The pattern shifts to the later hours for nighttime, with the 7th driving hours as the peak. Additionally, many incidents are recorded around 12 hours of night drives.
...
Here, drivers' workload is represented by the number of hours they have been driving within a day. Drivers' shifts dataset is expanded to a longer format so that each hour of driving can be noted. Further, variables measured hourly, e.g. temperature and precipitation, are suitable to be added to this newly established data set. Although the effects of temperature and precipitation on incidents have been verified earlier, they are now compared with other variables in regards to their relative importance when modeling. Thus, a random forest is built to examine the relevance of the four features, hours of driving, the hour of the day, temperature and precipitation, to the occurrence of incidents.

The response variable of 0 indicates that no incident was recorded during the given hour of the day and time, while 1 means the occurrence of an incident. Due to the imbalance between incidents and non-incidents which could lead to biased prediction, up-sampling is implemented to replicate data for the minority class to minimise the discrepancy in the results. 
...
Of the variables investigated in this analysis, all of them are associated with the occurrence of MVA incidents. Although `depot` has not proceeded to t-test, the gaps in between depots are clearly displayed. Noteworthily, a large portion of total MVA incidents occurred in more populated states, e.g. NSW and Melbourne. This is foreseeable as much research conducted globally also acknowledges the positive correlation between population and bus accidents ([Bentama, et al., 2017](https://doi.org/10.1109/LOGISTIQUA.2017.7962885)). The unexpectedly large number of drivers having incidents in Joondalup seems odd. Due to the high number of drivers recorded with slight incidents, we assume drivers in Melbourne, especially Revesby and Taren Point, might have been driving with less care or have more exposure to hazards.

Despite the t-test expects male drivers to be more likely to encounter an incident, the graphs reveal the unproportionate difference of incidents between male and female drivers. This suggests that there might be other complex factors within female drivers which have drawn to this result.

To some extent, drivers’ age and tenure are related. An old driver is not necessarily experienced. The statement that the youngest driver group (20-30) has been driving more safely is only partially true. These drivers might have been new to Transdev with less than 10 years of experience in bus driving, which is noted with the high occurrence of incidents. We can be sure that elder drivers’ driving performance is stable with the low numbers for drivers involved in incidents and the increase in tenure. However, they should be mindful of running into slight incidents.

Driving at night is commonly assumed with higher exposure to drink-driving, night visibility and fatigue effects ([Keall, et al., 2005](https://doi.org/10.1016/j.aap.2005.03.021.)). It is also polarised based on our findings from the literature review. Our findings claim the high risks of driving in the daytime, especially during peak traffic hours. Even drivers who had only been driving for less than 5 hours struggled to handle the busy traffic then. The possible factors mentioned above regarding nighttime driving should be considered as drivers who had already driven for 7 hours in the night are recorded with the high number of incidents.

Although temperature and precipitation both have an impact on incidents, temperature’s impact is assumed to be stronger. This is observed from the summary statistics where the numbers for rainfall are all less than the rainfall standard for rain (2.5 mm). However, drivers in Melbourne have been more likely to have incidents under heavy rain and high temperature than in Sydney.

# Recommendations

* Utilise tracking technology or system to track every trip so that common driving behaviours and places associated with danger can be spotted. This is suggested for drivers based in populated states - NSW and Melbourne (Revesby and Taren Point) and Joondalup in response to the unexpectedly high number of drivers involved in incidents.
...
* Provide extra training for newly joined drivers with less than 10 years of driving experience. Avoid assigning them to routes with high exposure to hazards or peak traffic hours (7-8 am and 3-5 pm on weekdays and 3 pm on weekends) at first. Assign them to depots with no drivers recorded with incidents for practice, e.g. Pyrmont, Randwick, Capalaba and CATS Perth.
* Regularly check with mid-age and elder drivers to ensure they are mindful of the slight incidents.
* Ensure drivers get enough rest prior to every night drive. Try to adjust the shift timetable so that drivers won’t be driving for over 7 hours at night. If the shift lasts longer than 5 hours, assign two drivers so that another driver can take over to avoid the fatigue effect or concentration issues. 
