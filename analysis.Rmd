...
How are drivers' characteristics (Depot, gender and age) affecting the incidents? Is there 
...
```{r}
test_x1 <- as.matrix(age_inc_tab %>%
                      ungroup() %>%
                      select(incident, `no incident`))
CLTest_age <- prop.test(test_x1)
tidy(CLTest_age)
```
...
```{r}
res.aov <- age_inc_tab %>% anova_test(prop ~ `age group`)
res.aov
...
#head(age_inc_tab)
#str(age_inc_tab)
#model_fit <- lm(prop ~ `age group`, data=age_inc_tab)
#summary(model_fit)
...
head(tmp_tash)
model_fit <- lm(binary_outcome ~ age_group, data=tmp_tash) #double check this is logistic regression, not linear regression
summary(model_fit)
# find anova function that gives p value breakdown - something like summary.aov()
```
...
down_train <- downSample(x = nsw_fit_tr[, -ncol(nsw_fit_tr)],
                         y = nsw_fit_tr$incident_indicator)
table(down_train$incident_indicator)   

nsw_fit_rf <- randomForest(incident_indicator ~ shift_hour_of_day+shift_hour_for_driver+Temperature+Precipitation, data = down_train, importance = TRUE)
print(nsw_fit_rf)

test_model <- lm(incident_indicator ~ shift_hour_of_day + shift_hour_for_driver + Temperature + Precipitation, data=down_train)
...
table(actual = nsw_fit_ts$incident_indicator, predicted = nsw_fit_ts_pred$.pred)

table(nsw_fit_tr$incident_indicator)
1689/(1689+40033)
