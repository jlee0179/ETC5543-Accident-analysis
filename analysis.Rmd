...
plot_ly(data = svr_dpt, x = ~Depot, color = ~CompanyID) %>%
  add_trace(y = ~serious) %>%
  add_trace(y = ~slight) %>%
  layout(
        ...
        buttons = list(
          list(method = "update",
               args = list(list(visible = list(TRUE, FALSE)),
                           list(yaxis = list(title = "Drivers had claims (%)"))),
               label = "serious"),
...
weather_incidents <- mel_syd_weather %>%
  inner_join(dh_incidents, by = c("company", "date_hour")) 
