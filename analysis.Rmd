...
nsw_fit_df <- long_shifts_inc %>%
  ...
  ungroup() %>%
  select(-EmployeeID, -has_incident, -shift_date, -shift_start_hour, -shift_end_hour, -trip_duration, -timestamp, -Depot, -CompanyID, -EmployeeStartDate, -EmploymentTerminationDate, -Position, -ContractType, -FTE, -BirthDate) %>% 
  mutate(Gender = case_when(Gender == "Male" ~ 1,
                            Gender == "Female" ~ 0))
...
nsw_fit_rf <- randomForest(incident_indicator ~ shift_hour_of_day+shift_hour_for_driver+Temperature+Precipitation+Gender, data = nsw_fit_tr, importance = TRUE)
print(nsw_fit_rf)

nsw_fit_ts_pred <- nsw_fit_ts %>%
  mutate(.pred = predict(nsw_fit_rf, nsw_fit_ts))
