...
long_shifts_inc <- nswshifts_lj_inc %>%
  ...
  #mutate(shift_start_hour = paste(shift_start_hour, "00", sep = ":"),
         #shift_start_hour = shift_start_hour %>% hm())
  #mutate(shift_hour_for_driver = difftime(shift_hour_of_day, shift_start_hour)) %>%
...
```{r}
long_shifts_inc$timestamp <- as_datetime(with(long_shifts_inc, paste(shift_date, shift_hour_of_day, sep = " ")), "%Y-%m-%d %H:%m:%s")
  # Use difftime to fix shift hour for driver
```
